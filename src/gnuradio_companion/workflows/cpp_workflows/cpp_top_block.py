...
class CppTopBlockGenerator(object):
    ...
    def __init__(self, flow_graph, output_dir):
        ...
        filename = self._flow_graph.get_option('id') # different from py counterpart.
    ...
    def write(self):
        ...
        self.title = fg.get_option('title') or fg.get_option(
            'id').replace('_', ' ').title()
        variables = fg.get_cpp_variables() # different from py counterpart.
        ...
        self.namespace = {
            ...
            'config': platform.config # different from py counterpart.
        }

        if not os.path.exists(self.file_path): # different from py counterpart.
            os.makedirs(self.file_path)

        for filename, data in self._build_cpp_header_code_from_template(): # different from py counterpart.
            with codecs.open(filename, 'w', encoding='utf-8') as fp:
                fp.write(data)

        if not self._generate_options.startswith('hb'): # different from py counterpart.
            if not os.path.exists(os.path.join(self.file_path, 'build')):
                os.makedirs(os.path.join(self.file_path, 'build'))

            for filename, data in self._build_cpp_source_code_from_template(): # different from py counterpart.
                with codecs.open(filename, 'w', encoding='utf-8') as fp:
                    fp.write(data)

            if fg.get_option('gen_cmake') == 'On': # different from py counterpart.
